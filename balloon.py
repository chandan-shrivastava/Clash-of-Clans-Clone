...
class Balloons:
    def __init__(self):
        self.attack = gs.BALLOON_ATTACK
        self.health = gs.BALLOON_HEALTH
        self.x = 0
        self.y = 0
    def spawn(self,inp):
        ...
        if gs.BALLOON_COUNT >= 10:
            ...
            gs.board[10][71] = Fore.WHITE + "L"
            self.x = 10
            self.y = 71
        elif inp == "n":
            gs.balloon_pos.append([25,21])
            gs.board[25][21] = Fore.WHITE + "L"
            self.x = 25
            self.y = 21
        elif inp == "m":
            gs.balloon_pos.append([41,41])
            gs.board[41][41] = Fore.WHITE + "L"
            self.x = 41
            self.y = 41

    def move(self):
        for trp in gs.balloon_pos:
            if self.health > 0:
                distance = 5001
                for buildng in gs.buildings_pos:
                    for p in buildng:
                        if gs.building[p[0]][p[1]][0] == "C" or gs.building[p[0]][p[1]][0] == "W":
                ...
                if distance != 5001:
                    if distance == 0: 
                        ...
                        if trp[1] < destination[1]:
                            ...
                            trp[1] = trp[1] - 1
                        gs.board[trp[0]][trp[1]] = Fore.WHITE + "L"
                        self.x = trp[0]
                        self.y = trp[1]
                else:
                    for buildng in gs.buildings_pos:
                    ...
                    if distance != 5001:
                        if distance == 0:
                            if gs.building[destination[0]][destination[1]] == "HUTS1":
